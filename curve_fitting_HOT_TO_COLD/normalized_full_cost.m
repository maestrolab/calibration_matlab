...
if filename == "original"
    ...
    eps_50 = eps_50 - min(eps_50);
    eps_100 = eps_100 - min(eps_100);
    eps_150 = eps_150 - min(eps_150);
    % eps_172 = eps_172 - min(eps_172);
    eps_200 = eps_200 - min(eps_200);
    ...
    % sigma_172 = 1e6 * 150*ones(size(sigma_172));
    sigma_200 = 1e6 * 200*ones(size(sigma_200));
    ...
    %     disp(delta_eps_150)
    %     disp(delta_eps_200)
        delta_eps_error = sqrt((delta_eps_50^2 + delta_eps_100^2 + delta_eps_150^2 + delta_eps_200^2)/5.);
...
if filename == "numerical_experimental_comparison"
    ...
    eps_num_M2 = eps_num_M2 - min(eps_num_M2);
        ...
        figure(1)
        clf(1)
        plot(T_F1,eps_num_F1,'r', 'linewidth',2)
        hold on
        plot(T_F2,eps_num_F2,'b', 'linewidth',2)
        plot(T_M1,eps_num_M1,'g', 'linewidth',2)
        plot(T_M2,eps_num_M2,'K', 'linewidth',2)
        
        plot(T_F1,eps_F1,'--r', 'linewidth',2)
        plot(T_F2,eps_F2,'--b', 'linewidth',2)
        plot(T_M1,eps_M1,'--g', 'linewidth',2)
        plot(T_M2,eps_M2,'--b', 'linewidth',2)

        % Root-mean squared error:
        output = sqrt(sum((eps_F1-eps_num_F1).^2)/numel(eps_F1));
        output = output + sqrt(sum((eps_F2-eps_num_F2).^2)/numel(eps_F2));
        output = output + sqrt(sum((eps_M1-eps_num_M1).^2)/numel(eps_M1));
        output = output + sqrt(sum((eps_M2-eps_num_M2).^2)/numel(eps_M2));

        if (initial_error == 0)
            initial_error = output;
        end

        delta_eps_M1 = (max(eps_M1) - min(eps_M1)) - (max(eps_num_M1) - min(eps_num_M1));
        delta_eps_M2 = (max(eps_M2) - min(eps_M2)) - (max(eps_num_M2) - min(eps_num_M2));
        delta_eps_F1 = (max(eps_F1) - min(eps_F1)) - (max(eps_num_F1) - min(eps_num_F1));
        delta_eps_F2 = (max(eps_F2) - min(eps_F2)) - (max(eps_num_F2) - min(eps_num_F2));
    %     disp(delta_eps_50)
    %     disp(delta_eps_100)
    %     disp(delta_eps_150)
    %     disp(delta_eps_200)
        delta_eps_error = sqrt((delta_eps_M1^2 + delta_eps_M2^2 + delta_eps_F1^2 + delta_eps_F2^2)/4.);
