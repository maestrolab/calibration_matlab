...
data_300 = textread('filtered_data_300MPa.txt');
...
sigma_200 = 1e6 * 150*ones(size(sigma_200));
sigma_300 = 1e6 * 200*ones(size(sigma_300));
...
[eps_num_150, MVF,eps_t,E,MVF_r,eps_t_r ] = Full_Model_stress( T_200, sigma_200, P, elastic_check, integration_scheme );
...
eps_num_200 = eps_num_150 - min(eps_num_150);
...
output = output + sqrt(sum((eps_200-eps_num_150).^2)/numel(eps_200));
...
delta_eps_200 = (max(eps_200) - min(eps_200)) - (max(eps_num_150) - min(eps_num_150));
